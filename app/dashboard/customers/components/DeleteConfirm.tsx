...
export default function DeleteConfirm({ customer, onClose, onDeleted }: Props) {
  ...
  const handleDelete = async () => {
    ...
    setError("");
    try {
      await apiRequest(`/api/customers/${customer._id}`, { method: "DELETE" });
    ...
    <ModalOverlay onClose={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm mx-4">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <h2 className="text-[14px] font-semibold text-slate-900">Delete Customer</h2>
          <button
            onClick={onClose}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-[13px] text-slate-800">
                Delete <span className="font-semibold">{customer.name}</span>?
              </p>
