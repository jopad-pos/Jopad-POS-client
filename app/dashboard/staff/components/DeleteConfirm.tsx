...
export default function DeleteConfirm({ staff, onClose, onDeleted }: Props) {
  ...
  async function handleDelete() {
    setLoading(true);
    try {
      await apiRequest(`/api/staff/${staff._id}`, { method: "DELETE" });
      onDeleted();
    } catch (err) {
    ...
    <ModalOverlay onClose={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm mx-4 p-5">
        ...
        <p className="text-[13px] text-slate-500 mb-4">
          Remove{" "}
          <span className="font-medium text-slate-700">{staff.name}</span>? This
          cannot be undone.
        </p>
