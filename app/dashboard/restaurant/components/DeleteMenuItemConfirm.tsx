...
export default function DeleteMenuItemConfirm({ item, onClose, onDeleted }: Props) {
  ...
  const handleDelete = async () => {
    setLoading(true);
    try {
      await apiRequest(`/api/menu-items/${item._id}`, { method: "DELETE" });
      onDeleted(item._id);
    } catch (err) {
  ...
  return (
    <ModalOverlay onClose={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm mx-4 p-5">
        <h2 className="text-[15px] font-semibold text-slate-800 mb-1">Delete Menu Item</h2>
        <p className="text-[13px] text-slate-500 mb-4">
          Remove <span className="font-medium text-slate-700">{item.name}</span> from your menu? This
          cannot be undone.
        </p>
