    ...
    <div className="bg-white border border-slate-200 rounded-lg flex flex-col flex-1 min-h-0">
      ...
      <div className="flex-1 overflow-auto min-h-0">
        {error ? (
          ...
          <div className="px-4 py-12 text-center text-[13px] text-slate-400">Loading menu…</div>
        ) : filtered.length === 0 ? (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100">
                {["Item", "Category", "Modifiers", "Price", ""].map((h) => (
                  <th
                    key={h}
            ...
            <tbody className="divide-y divide-slate-50">
              {paged.map((m) => (
                <tr key={m._id} className="hover:bg-slate-100 transition-colors group">
                  <td className="px-4 py-3">
                    <p className="text-[13px] font-medium text-slate-800 whitespace-nowrap">{m.name}</p>
                    {m.description && (
                  ...
                  <td className="px-4 py-3">
                    <span className="text-[12px] text-slate-500">{m.category || "—"}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-[12px] text-slate-500 whitespace-nowrap">
                      {m.modifierGroups.length ? `${m.modifierGroups.length} group(s)` : "—"}
                    </span>
                  </td>
                  <td className="px-4 py-3">
