...
export default function CloseOrderModal({ order, onClose, onClosed }: Props) {
  ...
  async function handleClose() {
    ...
    setError("");
    try {
      const updated = await apiRequest<Order>(`/api/orders/${order._id}/close`, {
        method: "POST",
        body: JSON.stringify({ method }),
      });
      onClosed(updated);
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "Failed to close the tab");
      setSaving(false);
    }
  }
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-slate-900 text-base font-semibold">Bill &amp; close tab</h2>
          </div>
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="space-y-2 text-sm">
            <Row label="Table" value={order.tableLabel} />
