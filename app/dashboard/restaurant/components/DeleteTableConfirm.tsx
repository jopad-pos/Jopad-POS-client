...
export default function DeleteTableConfirm({ table, onClose, onDeleted }: Props) {
  ...
  async function handleDelete() {
    ...
    setError("");
    try {
      await apiRequest(`/api/tables/${table._id}`, { method: "DELETE" });
      onDeleted(table._id);
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-slate-900/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h2 className="text-slate-900 text-base font-semibold">Delete Table {table.label}?</h2>
        </div>
        <p className="text-sm text-slate-500 mb-4">
          This table will be removed from your floor plan. Past orders are kept for your records.
        </p>
        {error && (
