...
export default function DeleteConfirm({ service, onClose, onDeleted }: Props) {
  ...
  const handleDelete = async () => {
    setLoading(true);
    try {
      await apiRequest(`/api/services/${service._id}`, { method: "DELETE" });
  ...
  return (
    <ModalOverlay onClose={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm mx-4 p-5">
        <h2 className="text-[15px] font-semibold text-slate-800 mb-1">Delete Service</h2>
        <p className="text-[13px] text-slate-500 mb-4">
          Remove <span className="font-medium text-slate-700">{service.name}</span> from your
          services list? This cannot be undone.
        </p>
