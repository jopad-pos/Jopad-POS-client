...
import { Quotation, ModalOverlay } from "./shared";
...
export default function DeleteConfirm({ quotation, onClose, onDeleted }: Props) {
  ...
  const handleDelete = async () => {
    ...
    setError("");
    try {
      await apiRequest(`/api/quotations/${quotation._id}`, { method: "DELETE" });
      onDeleted(quotation._id);
    } catch (err) {
    ...
    <ModalOverlay onClose={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm mx-4">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <h2 className="text-[14px] font-semibold text-slate-900">Delete Quotation</h2>
        ...
        <div className="px-5 py-4 space-y-3">
          ...
          <p className="text-[13px] text-slate-700">
            Delete <span className="font-semibold font-mono">{quotation.ref}</span> for{" "}
            <span className="font-semibold">{quotation.customer}</span>? This cannot be undone.
          </p>
        </div>
