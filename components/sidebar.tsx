...
export default function Sidebar() {
  ...
  const userRole = profile?.role === "client" ? "Owner" : profile?.staffRole ?? "Staff";
  ...
  function hasRoleAccess(featureKey?: FeatureKey): boolean {
    ...
    const allowed =
      profile?.staffPermissions?.[staffRole] ?? DEFAULT_STAFF_PERMISSIONS[staffRole] ?? [];
      ...
      <div className="px-4 py-4 border-b border-slate-200">
        ...
        <button className="w-full flex items-center justify-between bg-slate-50 hover:bg-slate-100 rounded-md px-3 py-2 transition-colors group">
          <div className="text-left min-w-0">
            ...
            {subtitle && (
              <p className="text-slate-500 text-[10px] mt-0.5 truncate">{subtitle}</p>
            )}
          </div>
              ...
              <div className="space-y-0.5">
                {visibleItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <item.icon className={`w-4 h-4 shrink-0 ${active && !locked ? "text-blue-600" : ""}`} />
      ...
      <div className="px-3 pb-3 border-t border-slate-200 pt-3 space-y-0.5">
        {bottomItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={`w-4 h-4 shrink-0 ${active ? "text-blue-600" : ""}`} />
              {item.label}
            </Link>
          );
        })}
        ...
        <div className="flex items-center gap-2.5 px-3 py-2 mt-2 rounded-md group hover:bg-slate-100 cursor-pointer transition-colors">
          <div className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center shrink-0">
            <span className="text-slate-700 text-[10px] font-semibold">{initials}</span>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-slate-900 text-[11px] font-medium truncate leading-none">
              {userName}
            </p>
            <p className="text-slate-500 text-[10px] truncate mt-0.5">{userRole}</p>
          </div>
